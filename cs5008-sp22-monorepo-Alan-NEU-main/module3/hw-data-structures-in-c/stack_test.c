...
// Simple enqueu and deque stack test
int unitTestAlan1(int status){
    int passed = 1;
    int cap = 13;
    int i;
    stack_t* test = create_stack(cap);
    // fill the stack
    for(i = 0; i < cap; i++){
        stack_enqueue(test, i+1);
    }
    //check full
    if(stack_full(test) != 1 || stack_size(test) != cap){
        passed = 0;
    }
    //dequeue all
    for(i = 0; i < cap; i++){
    ...
    //check empty
     if(stack_empty(test) != 1 || stack_size(test) != 0){
        passed = 0;
    }
    free_stack(test);
    return passed;
...
//insert and retrieve same value
int unitTestAlan2(int status){
    int passed = 1;
    int cap = 1;
    int value = 8;
    stack_t* test = create_stack(cap);
    stack_enqueue(test, value);
    if(stack_dequeue(test) != value){
        passed = 0;
    }
    free_stack(test);
    return passed;
...
//fill and dequeue, check every value is correct
int unitTestAlan3(int status){
    int passed = 1;
    int cap = 10;
    int i;
    stack_t* test = create_stack(cap);
    ...
    // dequeue and check stack
    for(i = cap; i > 0; i--){
        if(stack_dequeue(test) != i){
            passed = 0;
        }
    }
    free_stack(test);
